...
describe "show the depths of rspec" do
  ...
  context "work through the different types of Rspec matchers" do
    ...
    it 'should test that equality matchers are different' do
      ...
      expect(a).to equal(b) # object identity - a and b refer to the same object (int or string or float etc)
    ...
    it 'should test the various different type of comparison matcher' do
      ...
      expect(e).to be <= f # check to see if less than or equal to f
      expect(f).to be >= e # check to see if f is greater than or equal to e
      expect(e).to be_between(d, f) # check whether e is between d & f (exclusive of first value, inclusive of second value - can change this with .inclusive or .exclusive on the end)
